...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        output = self.attn(output) + output
        return output
    ...
    def step(self, x, cache):
        ...
        output = output + x
        output = self.attn(output.unsqueeze(1)).squeeze(1)
        return output, cache
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        # x: (B, L, D)
        query = self.query(x)  # (B, L, D)
        key = self.key(x)  # (B, L, D)
        value = self.value(x)  # (B, L, D)

        scores = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(query.size(-1))  # (B, L, L)
        attn_weights = self.softmax(scores)  # (B, L, L)
        attn_output = torch.matmul(attn_weights, value)  # (B, L, D)
